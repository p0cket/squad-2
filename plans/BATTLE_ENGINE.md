...
- **Build 1.0**: Critical damage flow fix ✅
  - Fixed missing `dispatch({ type: "UPDATE_CREATURE" })` in performAttack.ts
  - Creatures now properly lose health when attacked
- **Build 1.1**: Animation queue infrastructure ✅
  - Created AnimationQueue class with priority-based processing
  - Integrated with GameContext and React hooks
  - Added demo component for testing

### 🚧 **Phase 2: Integration (NEXT)**
- **Build 2.1**: Attack animation integration 🔲
  - Connect animation queue to actual attack flow
  - Replace immediate animations with queue operations
- **Build 2.2**: Status effect unification 🔲
  - Merge `mods` and `statuses` into single effect system

### ⏳ **Future Phases**
- **Phase 3**: Polish (Battle controller, error handling, visual feedback)
- **Phase 4**: Optimization (Performance, testing suite)
...
1. **Attack Damage**: Execute attack → target health decreases
2. **Animation Queue**: Use demo component → queue length updates
3. **Status Effects**: Apply poison → damage over time works
4. **Console Logs**: Check for error-free operation

### Animation Queue Demo
- Add test animations and verify queue length increases
- Process queue and watch console for execution logs
- Clear queue to reset state
...
### Core Battle Logic
- `src/utils/moves/performAttack.ts` - Main attack processing
- `src/utils/moves/calculateDamageAndStatuses.ts` - Damage computation
- `src/utils/party/updateTargetState.ts` - Pure state updates

### Animation System
- `src/utils/anim/AnimationQueue.ts` - Queue management
- `src/hooks/useAnimationQueue.ts` - React integration
- `src/components/AnimationQueueDemo.tsx` - Testing component

### State Management
- `src/GameContext.tsx` - Central state container
- `src/consts/types/types.ts` - TypeScript interfaces
- `src/consts/statuses.ts` - Status effect definitions
...
### Common Issues
1. **Creatures don't lose health** → Check dispatch call in performAttack.ts
2. **Queue buttons don't work** → Verify AnimationQueue returns new instances
3. **Status effects don't apply** → Check timing and dispatch calls
4. **Performance issues** → Use React.memo and memoization

### Debug Tools
- Animation Queue Demo component for testing
- React DevTools for state inspection
- Browser console for detailed logging
- Performance tab for frame rate monitoring
